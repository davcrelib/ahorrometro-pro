...
export default function HomePage() {
  ...
  // Botones con micro-interacciones y mejor accesibilidad
  const btnBase =
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#0a0f14] via-[#0c131a] to-[#0a0f14] text-white selection:bg-emerald-400/20 selection:text-white">
      ...
      <nav className="sticky top-0 z-40 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between backdrop-blur supports-[backdrop-filter]:bg-white/5 bg-white/[0.02] border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold tracking-tight text-white/95">Ahorrómetro</span>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <button
            onClick={() => handleCTA("/app")}
            className={btnGhost + " hidden xs:inline-flex sm:inline-flex"} // visible desde sm, si quieres siempre visible quita 'hidden sm:inline-flex'
          >
            Ir al panel
          </button>
          <Link
            ...
            <div className="relative max-w-[680px] mx-auto w-full">
              <div className="rounded-3xl border border-white/10 bg-gradient-to-b from-white/10 to-white/5 shadow-2xl p-4 backdrop-blur supports-[backdrop-filter]:bg-white/5">
                ...
                <div className="rounded-2xl bg-black/40 border border-white/10 p-4 md:p-5">
                  ...
                  <div className="text-3xl sm:text-4xl font-bold mt-1">1.257,34 €</div>

                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl bg-white/5 border border-white/10 p-3">
                      <div className="text-xs text-white/70">€ / hora</div>
                      <div className="text-lg sm:text-xl font-semibold">15,62 €</div>
                    </div>
                    <div className="rounded-xl bg-white/5 border border-white/10 p-3">
                      <div className="text-xs text-white/70">Presupuesto semanal</div>
                      <div className="text-lg sm:text-xl font-semibold">185,00 €</div>
                    </div>
                  </div>
              ...
              <div className="absolute -bottom-6 -left-6 h-24 w-24 rounded-full blur-3xl bg-emerald-400/30" aria-hidden />
              <div className="absolute -top-6 -right-8 h-24 w-24 rounded-full blur-3xl bg-sky-400/30" aria-hidden />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        <div className="text-center text-white/75 text-sm">Con hábitos reales, no magia ✨</div>
      </section>
...
// ====== Datos estáticos ======
const features = [
  { emoji: "⏱️", title: "Dinero hora a hora", desc: "Ve cuánto llevas acumulado este mes en tiempo real. Motivación diaria sin fórmulas raras." },
  { emoji: "🧾", title: "Gastos en 3 toques", desc: "Apunta cualquier gasto al momento. Categorías simples y sin complicaciones." },
  { emoji: "📆", title: "Límite semanal automático", desc: "El sistema calcula cuánto puedes gastar esta semana para no romper tu objetivo." },
  { emoji: "🎯", title: "Objetivo (ej. entrada piso)", desc: "Marca tu meta (ej. 150.000 €) y fecha. Calculamos el ahorro mensual que necesitas." },
  { emoji: "☁️", title: "Sincronización (Pro)", desc: "Tus datos en la nube: iPhone y portátil siempre iguales. Backups automáticos." },
  { emoji: "📈", title: "Métricas claras", desc: "€ / hora, acumulado del mes, progreso y tendencia semanal. Todo a simple vista." },
];

const steps = [
  { title: "Crea tu plan", desc: "Introduce sueldo, gastos fijos, ahorros y la fecha objetivo." },
  { title: "Apunta tus gastos", desc: "Anótalos al momento. El límite semanal se ajusta automáticamente." },
  { title: "Sigue el progreso", desc: "Revisa tu acumulado y cómo te acercas a tus objetivos." },
];
